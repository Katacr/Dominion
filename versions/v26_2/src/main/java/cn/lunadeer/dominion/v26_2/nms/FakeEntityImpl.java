 ...
 * <p>
 * Minecraft 26.2 no longer exposes display entity types as static fields on
 * {@link EntityType}, so display entity types are resolved from the built-in
 * registry by key.
 */
@SuppressWarnings("unchecked")
public class FakeEntityImpl implements FakeEntity {

    /** Client-side BlockDisplay entity type used for fake border blocks. */
    private static final EntityType<?> BLOCK_DISPLAY_TYPE = resolveEntityType("block_display");

    /** Client-side ItemDisplay entity type used for fake item displays. */
    private static final EntityType<?> ITEM_DISPLAY_TYPE = resolveEntityType("item_display");
    ...
    /**
     * Resolves a vanilla entity type from the built-in registry.
     * <p>
     * This avoids binary linkage to removed or signature-changed static fields
     * such as {@code EntityType.BLOCK_DISPLAY} on Minecraft 26.2.
     */
    private static EntityType<?> resolveEntityType(String key) {
        ...
        if (entityType == null) {
            throw new IllegalStateException("Missing Minecraft entity type: minecraft:" + key);
        }
        return entityType;
    }
